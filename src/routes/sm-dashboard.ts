...
adminSmDashboardRouter.get("/", async (req, res, next) => {
  try {
    ...
    const scopedWhere = whereClause(scopeConditions(parsed.data));
    const [visitResult, oosResult, filterOptions] = await Promise.all([
      db.execute(sql<VisitDbRow>`
        select
